#!/usr/bin/env ruby
# Test script for new WhatsApp improvements (image captions + price shortcuts)
...
def main
  log("=" * 50, GREEN)
  log("WhatsApp Improvements Test", GREEN)
  log("=" * 50, GREEN)
  log("")
  ...
  # Send title with detailed description (simulating image caption)
  log("Step 4: Sending title with detailed description", YELLOW)
  log("-" * 50, YELLOW)
  
  detailed_title = "Apple MacBook Pro 14-inch M3 Chip 16GB RAM 512GB SSD. Excellent condition, used for 6 months. Includes original charger and box. Battery health 95%."
  
  result = send_webhook_text(SELLER_PHONE, detailed_title)
  unless result[:success]
    log("✗ Failed to send title", RED)
    return
  end
  
  log("✓ Title with description sent", GREEN)
  sleep(3)
  ...
  log("  Description: #{product_data['description']&.first(100)}...", GREEN)
  log("")
  
  # Trigger edit mode
  log("Step 6: Triggering edit mode to test price selection", YELLOW)
  log("-" * 50, YELLOW)
  ...
  log("Step 7: Triggering price selection", YELLOW)
  log("-" * 50, YELLOW)
  
  result = send_webhook_text(SELLER_PHONE, 'PRICE')
  unless result[:success]
    log("✗ Failed to send PRICE command", RED)
    return
  end
  
  log("✓ PRICE command sent", GREEN)
  log("")
  
  # Simulate button response for 50K-100K range
  log("Step 8: Simulating button response for '50K-100K' range", YELLOW)
  log("-" * 50, YELLOW)
  
  result = send_webhook_button(SELLER_PHONE, 'price_50k_100k')
  unless result[:success]
  ...
  log("Session step: #{session.step}", GREEN)
  log("Final price: KES #{product_data['price']}", GREEN)
  log("Final description length: #{product_data['description']&.length || 0} characters", GREEN)
  log("")
