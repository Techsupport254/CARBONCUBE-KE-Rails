...
class SalesBrandActivity < ApplicationRecord
  belongs_to :sales_brand
  belongs_to :sales_user, optional: true

  # A single touchpoint with a brand. A brand can be called or visited many
  # times — each interaction is a row, with what they said captured in notes.
  enum :activity_type, {
    call: 0,
    visit: 1,
    whatsapp: 2,
