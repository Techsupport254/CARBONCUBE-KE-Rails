class SendShopIdentityOptimizedJob < ApplicationJob
  queue_as :default

  def perform
    Rails.logger.info "=== SHOP IDENTITY OPTIMIZED JOB START ==="
    
    # Get all active sellers with phone numbers
    ...
    if sellers_to_process.none?
      Rails.logger.info "All sellers have already received the shop_identity template"
      Rails.logger.info "=== SHOP IDENTITY OPTIMIZED JOB COMPLETED ==="
      return
    end
    ...
    Rails.logger.info "=== SHOP IDENTITY OPTIMIZED JOB SUMMARY ==="
