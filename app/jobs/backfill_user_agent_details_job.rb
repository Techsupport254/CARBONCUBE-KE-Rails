class BackfillUserAgentDetailsJob < ApplicationJob
  queue_as :default
  ...
  def perform(batch_size: 1000)
    ...
    Rails.logger.info "BackfillUserAgentDetailsJob: Starting backfill for #{total_count} records"
    ...
    base_query.find_each(batch_size: batch_size) do |click_event|
      begin
        ...
        if processed % 100 == 0
          Rails.logger.info "BackfillUserAgentDetailsJob: Processed #{processed}/#{total_count} records"
        end
      rescue StandardError => e
        errors += 1
        Rails.logger.error "BackfillUserAgentDetailsJob: Error processing click_event #{click_event.id}: #{e.message}"
        Rails.logger.error e.backtrace.join("\n")
      end
    end
    
    Rails.logger.info "BackfillUserAgentDetailsJob: Completed! Processed: #{processed}, Errors: #{errors}, Total: #{total_count}"
