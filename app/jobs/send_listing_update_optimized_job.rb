class SendListingUpdateOptimizedJob < ApplicationJob
  ...
  def perform
    Rails.logger.info "=== LISTING UPDATE OPTIMIZED JOB START ==="
    ...
    Rails.logger.info "Total active sellers with phones: #{all_sellers.count}"
    ...
    if sellers_to_process.none?
      ...
      Rails.logger.info "=== LISTING UPDATE OPTIMIZED JOB COMPLETED ==="
      return
    end
    ...
    sellers_to_process.find_each do |seller|
      begin
        ...
        # Delay for faster processing (0.1 seconds = ~600 messages/minute)
        sleep(0.1)
    ...
    Rails.logger.info "=== LISTING UPDATE OPTIMIZED JOB SUMMARY ==="
