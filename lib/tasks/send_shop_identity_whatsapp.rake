namespace :admin do
  desc "Send WhatsApp shop identity template to seller (TEST MODE: only optisoftkenya@gmail.com)"
  task send_shop_identity_whatsapp: :environment do
    # TEST MODE: Only send to optisoftkenya@gmail.com
    seller_email = ENV.fetch('SELLER_EMAIL', 'optisoftkenya@gmail.com')
    
    puts "=== SHOP IDENTITY WHATSAPP RAKE TASK ==="
    ...
    # Enqueue the job
    job = SendShopIdentityWhatsAppJob.perform_later(seller_email)
  ...
  desc "Send WhatsApp shop identity template to all sellers (optimized)"
  task send_shop_identity_to_all_sellers: :environment do
    puts "=== SHOP IDENTITY TO ALL SELLERS RAKE TASK ==="
    puts "=============================================="
    ...
    if remaining == 0
      puts "✅ All sellers have already received the shop_identity template!"
    ...
    job = SendShopIdentityOptimizedJob.perform_later
