#!/usr/bin/env ruby
# Test script for WhatsApp Apple MacBook image upload with correct title
...
def main
  log("=" * 50, GREEN)
  log("WhatsApp Apple MacBook Test", GREEN)
  log("=" * 50, GREEN)
  log("")
  ...
  # Upload images
  log("Step 1: Uploading MacBook images to Cloudinary", YELLOW)
  log("-" * 50, YELLOW)
  ...
  unless product_data['suggested_price']
    log("Setting price to KES 185,000", YELLOW)
    session.update_product_data('price', 185000)
    session.update_product_data('suggested_price', 185000)
  end
