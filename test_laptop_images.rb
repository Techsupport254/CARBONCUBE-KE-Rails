#!/usr/bin/env ruby
# Test script for WhatsApp laptop image upload with correct title
...
def main
  log("=" * 50, GREEN)
  log("WhatsApp Laptop Image Test", GREEN)
  log("=" * 50, GREEN)
  log("")
  ...
  # Upload images
  log("Step 1: Uploading laptop images to Cloudinary", YELLOW)
  log("-" * 50, YELLOW)
  ...
  # Send LAPTOP title (not phone title)
  log("Step 4: Sending laptop title", YELLOW)
  log("-" * 50, YELLOW)
  
  result = send_webhook_text(SELLER_PHONE, 'Dell Latitude 7490 Laptop i5 8GB 256GB SSD')
  unless result[:success]
  ...
  # Check session status after AI analysis
  ...
  log("  Detected Objects: #{product_data['detected_objects']&.join(', ')}", GREEN)
  log("")
  
  # Set price if needed
  unless product_data['suggested_price']
    log("Setting price to KES 45,000", YELLOW)
    session.update_product_data('price', 45000)
    session.update_product_data('suggested_price', 45000)
  end
  
  # Confirm product creation
  log("Step 6: Confirming product creation", YELLOW)
  log("-" * 50, YELLOW)
  ...
  log("Step 7: Verifying product creation", YELLOW)
  log("-" * 50, YELLOW)
  ...
  if session.status == 'completed'
    ...
    if recent_ad
      ...
      log("  Manufacturer: #{recent_ad.manufacturer}", GREEN)
